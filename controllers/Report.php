...
class Report extends BaseController {
 ...
 private function parcelQuantityByMonth($month) {
        ...
        $listin = $this->db->read("SELECT day(parcel_indate) as m,count(*) as c FROM `parcel` WHERE month(parcel_indate)=$month and parcel_status != 0 group by date(parcel_indate) order by date(parcel_indate) ");
        $listout = $this->db->read("SELECT day(parcel_outdate) as m,count(*) as c FROM `parcel` WHERE month(parcel_outdate)=$month and parcel_status = 3 group by date(parcel_outdate) order by date(parcel_outdate) ");
